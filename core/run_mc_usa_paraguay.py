#!/usr/bin/env python3
"""Run 100k MC simulation for USA vs Paraguay"""
import json, math
import numpy as np
...
min_teams = {'Canada', 'Bosnia', 'USA', 'Paraguay', 'Mexico', 'Argentina', 'Brazil', 'England', 'France', 'Germany', 'Spain', 'Portugal'}
for mt in min_teams:
    if mt in team_counts:
        top_names.add(mt)
print(f'Teams in fit: {len(top_names)}')
...
usa_i=t2i['USA']; par_i=t2i['Paraguay']
lam_h=np.exp(attack[usa_i]+defense[par_i])
lam_a=np.exp(attack[par_i]+defense[usa_i])
print(f'\n=== DC EXPECTED GOALS ===')
print(f'USA: {lam_h:.4f}  Paraguay: {lam_a:.4f}')
...
print(f'   USA vs Paraguay')
print(f'{"="*55}')
print(f'\nRESULT:')
print(f'USA Win:   {hw:.1%}  {"#"*int(hw*80)}')
print(f'Draw:      {drw:.1%}  {"#"*int(drw*80)}')
print(f'Paraguay:  {aw:.1%}  {"#"*int(aw*80)}')
...
for d in sorted(marg):
    p=marg[d]/N
    if p>0.005:
        l=f'USA+{d}' if d>0 else (f'Par+{-d}' if d<0 else 'Draw')
        print(f'  {l:>10s}: {p:.1%}  {"#"*int(p*100)}')

print(f'\nBEST PICK: USA {sorted(sc.items(),key=lambda x:-x[1])[0][0]} ({sorted(sc.items(),key=lambda x:-x[1])[0][1]/N:.1%})')
...
print(f'\nDETAIL:')
print(f'  USA win 2+: {w2p:.1%}')
print(f'  USA win 1:  {w1:.1%}')
print(f'  Draw:       {drw:.1%}')
print(f'  Par win 1:  {l1:.1%}')
print(f'  Par win 2+: {l2p:.1%}')
print(f'\nDone: {N:,} simulations')
