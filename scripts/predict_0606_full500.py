...
for no, meta in TARGET_MATCHES.items():
    ...
    if model_ok:
        ...
        p_model = {
            '胜': spf_market.get('胜', 1/3),
            '平': spf_market.get('平', 1/3),
            '负': spf_market.get('负', 1/3),
        }
        jqs_mean = 0.0
        jqs_raw = jqs.get(no, {}).get('odds', {})
        tg_market = sp_to_prob_map(jqs_raw)
        for label, prob in tg_market.items():
    ...
    tg_market = {}
    for k, v in jqs_market.items():
        nk = 7 if k == '7+球' else int(k.replace('球',''))
        tg_market[nk] = v
    total_dist = {}
    for k in range(8):
        total_dist[k] = 0.65*tg_model.get(k, 0.0) + 0.35*tg_market.get(k, 0.0)
    ...
    htft_merged = {k: 0.65*htft_model.get(k, 0.0) + 0.35*htft_market.get(k, 0.0) for k in HTFT_LABELS}
    htft_top = sorted(htft_merged.items(), key=lambda x: -x[1])
...
with open('/root/scripts/preds_0606_full500.json', 'w', encoding='utf-8') as f:
    json.dump(predictions, f, ensure_ascii=False, indent=2)
print('/root/scripts/preds_0606_full500.json')
